...
MESSY_SCHEMAS: dict[str, list[dict]] = {
    "sales": [
        {"name": "orders", "description": "Customer order transactions with payment and fulfillment status.",
         "columns": [("order_id", "BIGINT", None),
                     ("customer_id", "BIGINT", "PII.NonSensitive"),
                     ("total_amount", "DECIMAL", None),
                     ("created_at", "TIMESTAMP", None)]},
        {"name": "refund_events", "description": "Daily sales aggregates by region and product.",
         "columns": [("refund_id", "BIGINT", None),
                     ("order_id", "BIGINT", None),
                     ("reason_code", "STRING", None),
                     ("refunded_at", "TIMESTAMP", None)]},
        {"name": "cart_abandonments", "description": "",
         "columns": [("session_id", "STRING", None),
                     ("cust_id", "BIGINT", None),
                     ("abandoned_at", "TIMESTAMP", None)]},
    ],
    "marketing": [
        {"name": "campaign_attr", "description": "table for campaigns",
         "columns": [("campaign_id", "BIGINT", None),
                     ("CustomerID", "BIGINT", None),
                     ("channel", "STRING", None),
                     ("email", "STRING", None)]},
        {"name": "email_sends", "description": "Outbound email campaign send events with delivery status.",
         "columns": [("send_id", "BIGINT", None),
                     ("email", "STRING", "PII.Sensitive"),
                     ("campaign_id", "BIGINT", None)]},
    ],
    "users": [
        {"name": "customer_profiles", "description": "Master customer dimension with demographic attributes.",
         "columns": [("id", "BIGINT", None),
                     ("email", "STRING", "PII.Sensitive"),
                     ("first_name", "STRING", "PII.Sensitive"),
                     ("phone", "STRING", None)]},
        {"name": "user_sessions", "description": "",
         "columns": [("session_id", "STRING", None),
                     ("cid", "BIGINT", None),
                     ("started_at", "TIMESTAMP", None)]},
    ],
    "finance": [
        {"name": "invoices", "description": "Accounts receivable invoice line items.",
         "columns": [("invoice_id", "BIGINT", None),
                     ("customer_id", "BIGINT", None),
                     ("amount", "DECIMAL", None)]},
        {"name": "payments", "description": "t",
         "columns": [("payment_id", "BIGINT", None),
...
def ensure_schema(c: httpx.Client, schema: str) -> None:
    r = c.put("/v1/databaseSchemas", json={
        "name": schema,
...
def create_table(c: httpx.Client, schema: str, spec: dict) -> None:
    cols = [
        {"name": name, "dataType": dtype, "tags":
            [{"tagFQN": tag, "labelType": "Manual", "state": "Confirmed", "source": "Classification"}]
            if tag else []}
